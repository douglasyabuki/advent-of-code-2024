export const day = {
  ...
  story: `The Chief Historian is always present for the big Christmas sleigh launch, but nobody has seen him in months! Last anyone heard, he was visiting locations that are historically significant to the North Pole; a group of Senior Historians has asked you to accompany them as they check the places they think he was most likely to visit.
  ...
  firstStar: (input: string): number => {
    ...
    input.split("\n").forEach((item) => {
      if (item.trim()) {
        const [before, after] = item.trim().split(emptySpaceRegex);
        numbersBefore.push(+before);
        numbersAfter.push(+after);
      }
    });
    numbersBefore.sort();
    numbersAfter.sort();
    return numbersBefore.reduce(
      (acc, before, i) => acc + Math.abs(before - numbersAfter[i]),
      0,
    );
  },
  secondStar: (input: string): number => {
    ...
    for (const num of numbersAfter) {
      countMap.set(num, (countMap.get(num) || 0) + 1);
    }
    let result = 0;
    for (const n of numbersBefore) {
      result += n * (countMap.get(n) || 0);
    }
